...
每周六08:00更新按公告日保存的财务历史，并固化一份带日期的行业成员快照。
...
sudo systemctl list-timers winstock-update.timer winstock-morning.timer winstock-enrich.timer --all
...
```bash
sudo systemctl disable --now winstock-update.timer winstock-morning.timer winstock-enrich.timer
```
